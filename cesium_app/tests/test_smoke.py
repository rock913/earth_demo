...
def test_config_has_required_settings():
    ...
    # locations 应该是字典且包含基本城市
    assert isinstance(settings.locations, dict)
    assert "xiongan" in settings.locations
    assert "shanghai" in settings.locations
    
    # modes 应该是字典且包含4种模式
    assert isinstance(settings.modes, dict)
    assert len(settings.modes) == 4
    assert all(mode in settings.modes for mode in ["dna", "change", "intensity", "eco"])

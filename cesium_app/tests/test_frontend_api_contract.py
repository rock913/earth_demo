...
class TestModesAPIContract:
    ...
    def test_modes_returns_dict_format(self, client):
        ...
        # 验证至少包含4个AI模式
        assert len(data) >= 4, "应该至少有4个AI模式"
        
        # 验证必须包含的模式
        required_modes = ["dna", "change", "intensity", "eco"]
...
class TestLocationsAPIContract:
    ...
    def test_location_structure_is_correct(self, client):
        ...
        # 验证至少有 xiongan
        assert "xiongan" in data, "应该包含雄安地点"
        
        xiongan = data["xiongan"]
        
        # 前端需要的字段
        assert "name" in xiongan, "地点应该有 name 字段"
        assert "coords" in xiongan, "地点应该有 coords 字段"
        assert "bounds" in xiongan, "地点应该有 bounds 字段用于 Cesium 视口"
        
        # 验证 coords 格式: [lat, lon, zoom]
        assert isinstance(xiongan["coords"], list), "coords 应该是数组"
        assert len(xiongan["coords"]) == 3, "coords 应该有3个元素: [lat, lon, zoom]"
        
        # 验证 bounds 格式
        bounds = xiongan["bounds"]
...
class TestFrontendWorkflow:
    ...
    def test_location_change_workflow(self, client):
        ...
        selected_location_id = "xiongan"  # ✅ 正确：使用ID
    ...
    def test_cache_export_workflow(self, client):
        ...
        """
        selected_mode = "dna"  # ✅ 字符串ID
        selected_location = "xiongan"

"""TDD tests for V5 Dynamic Zonal Statistics API.
...
class TestStatsEndpoint:
    def test_stats_rejects_invalid_mode(self, client: TestClient):
        resp = client.post("/api/stats", json={"mode": "nope", "location": "xiongan"})
        assert resp.status_code == 400

    def test_stats_rejects_invalid_location(self, client: TestClient):
        resp = client.post("/api/stats", json={"mode": "dna", "location": "nope"})
        assert resp.status_code == 400
    ...
    def test_stats_happy_path(
        ...
        mock_img = Mock()
        mock_get_layer_logic.return_value = (mock_img, {"min": 0, "max": 1}, "dna")
        ...
        resp = client.post("/api/stats", json={"mode": "change", "location": "xiongan"})
        assert resp.status_code == 200
        data = resp.json()

        assert data["mode"] == "change"
        assert data["location"] == "xiongan"

...
class TestAPIEndpoints:
    ...
    def test_modes_endpoint_returns_all_modes(self):
        ...
        # 应该有4个模式
        assert len(data) == 4, f"应该有4个模式，实际: {len(data)}"
...
class TestLayersEndpoint:
    ...
    @pytest.mark.parametrize("mode", ["dna", "change", "intensity", "eco"])
    def test_all_modes_return_valid_data(self, mode):
        """测试：所有模式都应该返回有效数据"""
        response = requests.get(
            f"{BASE_URL}/api/layers",
            params={"mode": mode, "location": "xiongan"},
            timeout=30
        )
    ...
    def test_different_locations_return_different_bounds(self):
        """测试：不同位置应该返回不同的边界"""
        locations = ["xiongan", "shanghai"]
        bounds_list = []
        
        for location in locations:
            response = requests.get(
                f"{BASE_URL}/api/layers",
                params={"mode": "dna", "location": location},
                timeout=30
            )
...
class TestPerformance:
    ...
    def test_concurrent_requests_dont_crash(self):
        ...
        # 并发发送 4 个请求（每个模式一个）
        modes = ["dna", "change", "intensity", "eco"]

...
def test_modes_endpoint():
    ...
    assert "地表 DNA (语义视图)" in data.values()
    assert "变化雷达 (敏捷治理)" in data.values()
    assert "建设强度 (宏观管控)" in data.values()
    assert "生态韧性 (绿色底线)" in data.values()
...
def test_locations_endpoint():
    ...
    # 验证雄安位置存在
    assert "xiongan" in data
    # coords 格式是 [lat, lon, zoom]
    assert data["xiongan"]["coords"][0] == 39.05  # lat
    assert data["xiongan"]["coords"][1] == 115.98  # lon
...
def test_dna_mode_specific():
    ...
    response = requests.get(
        f"{BASE_URL}/api/layers",
        params={"mode": "dna", "location": "xiongan"},
        timeout=30
    )
